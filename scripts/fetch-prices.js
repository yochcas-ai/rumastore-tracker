...
async function fetchEbay(carName) {
  ...
  try {
    const q = encodeURIComponent(`Hot Wheels ${carName} Pop Culture 2026`);
...
async function fetchMercadoLibre(carName) {
  try {
    const q   = encodeURIComponent(`Hot Wheels ${carName} 2026`);
    ...
    const res  = await fetch(url, {
      headers: { "User-Agent": "RumaStoreBot/1.0" }
    });
    if (!res.ok) return null;
    const json = await res.json();
    ...
    console.log(`    MercadoLibre [${carName}]: ${prices.length} listados → S/ ${median(prices)}`);
    return {
      median:  +median(prices).toFixed(2),
      low:     prices[Math.floor(prices.length * 0.25)],
      high:    prices[Math.floor(prices.length * 0.75)],
...
async function main() {
  ...
  for (const car of CARS) {
    ...
    const sources = {
      ebay:         ebay   ? { ...ebay,  median: +(ebay.median * TC).toFixed(2),
                                          low:    +(ebay.low   * TC).toFixed(2),
                                          high:   +(ebay.high  * TC).toFixed(2),
                                          median_usd: ebay.median } : null,
      koban:        koban  || null,
    ...
    results[car.upc] = {
      ...
      confidence,
      sources: {
        ebay:         sources.ebay         ? { median_soles: sources.ebay.median,         median_usd: sources.ebay.median_usd, samples: ebay.samples } : null,
        koban:        sources.koban        ? { median_soles: sources.koban.median,         samples: koban.samples }  : null,
        mercadolibre: sources.mercadolibre ? { median_soles: sources.mercadolibre.median,  samples: ml.samples }     : null,
      },
