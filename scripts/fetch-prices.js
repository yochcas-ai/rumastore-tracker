...
/**
 * fetch-prices.js — Ruma Store · Hot Wheels Market Tracker
  * eBay Browse API + Koban.pe + MercadoLibre Perú (requiere ML_ACCESS_TOKEN)
   *
    * Secrets requeridos en GitHub Actions:
     *   EBAY_CLIENT_ID      → App ID de producción eBay
      *   EBAY_CLIENT_SECRET  → Cert ID de producción eBay
       *   ML_ACCESS_TOKEN     → Bearer token de MercadoLibre (app libre en developers.mercadolibre.com)
        */
      import fs from "fs";
    import path from "path";
    ...
    /**
     * FIX: confianza basada en fuentes + muestras.
      * 3 fuentes → "high"
       * 2 fuentes → "medium"
        * 1 fuente con ≥3 muestras → "medium"
         * 1 fuente con <3 muestras → "low"
          * 0 fuentes → "none"
           */
    function calcConfidence(sources) {
        ...
        // 1 fuente — evaluar por cantidad de muestras
    ...
    async function getEbayToken() {
        ...
        try {
              ...
              const data = await res.json();
              _ebayToken = { value: data.access_token, expiresAt: Date.now() + (data.expires_in - 60) * 1000 };
              console.log(`  eBay token OK (expira en ${data.expires_in}s)`);
              return _ebayToken.value;
        } catch(e) {
              console.warn("  eBay token fetch error:", e.message);
    ...
    async function fetchEbay(carName) {
        ...
        try {
              ...
              const url = `https://api.ebay.com/buy/browse/v1/item_summary/search`
                      + `?q=${q}`
                      + `&filter=conditions%3ANEW%2CpriceRange%3A%5B3..200%5D`
                      + `&sort=price`
                      + `&limit=50`;
              const res = await fetch(url, {
                      headers: {
    ...
    // ─── MERCADOLIBRE PERÚ ───────────────────────────────────────────────────────
    // Requiere ML_ACCESS_TOKEN (Bearer token de app registrada en developers.mercadolibre.com)
    // Registro gratuito: https://developers.mercadolibre.com.pe → Crear app → client_credentials
    async function fetchMercadoLibre(carName) {
        if (!ML_ACCESS_TOKEN) return null;   // sin token, saltar silenciosamente
        try {
              ...
              const res   = await fetch(url, {
                      headers: {
                                ...
                                "User-Agent":    "RumaStoreBot/1.0",
                                "Accept":        "application/json",
                      }
              });
              if (!res.ok) {
                      console.warn(`  ML HTTP ${res.status} [${words}]`);
                      return null;
              }
              const json  = await res.json();
    ...
    async function main() {
        ...
        const hasMl   = !!ML_ACCESS_TOKEN;
      
        console.log(`\n🚗 Ruma Store Price Fetcher — ${new Date().toISOString()}`);
        console.log(`📡 eBay${hasEbay?" ✓":" ✗ (falta EBAY_CLIENT_SECRET)"} | Koban ✓ | ML${hasMl?" ✓":" ✗ (falta ML_ACCESS_TOKEN)"}\n`);
        ...
        const output = {
              ...
              tc:             TC,
              sources_active: { ebay: hasEbay, koban: true, mercadolibre: hasMl },  // FIX: refleja realidad
              prices:         results,
              cars:           CARS,
        };
